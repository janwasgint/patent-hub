#!/bin/bash
rm -rf src/const.js
truffle compile && truffle migrate --reset > parse_PH_address
address=`cat parse_PH_address | grep "PatentHub: " | cut -f 4 -d " "`
str_out="const deployedAddress = '$address';"
echo ${str_out} > src/const.js
echo "const keyMappings = {
...
export { deployedAddress, keyMappings };" >> src/const.js
#rm -rf parse_PH_address
